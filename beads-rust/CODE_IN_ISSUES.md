...
> Guidance on putting code snippets in beads_rust (br) issues.
...
```bash
br update br-42 --notes "WORKING CODE (tested):
\`\`\`python
...
```bash
br update br-42 --design "Call the API and parse response"
```
...
```bash
br update br-42 --notes "API response: {150 lines of JSON...}"
```
...
```bash
br update br-17 --design "Open config.yaml, change 'teh' to 'the'"
```
